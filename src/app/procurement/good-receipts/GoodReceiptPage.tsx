...
import CreateGoodReceiptModal from "@/components/atoms/modals/create/CreateGoodReceiptModal";
import { TableGoodReceipt } from "@/components/atoms/table/tableGoodReceipt";
import { Column, ListResponse } from "@/types/api";
import { GoodReceiptStatus } from "@/types/purchaseItem";
...
const STATUS_RECEIPT: GoodReceiptStatus[] = [
  GoodReceiptStatus.DRAFT,
  GoodReceiptStatus.PARTIAL,
  GoodReceiptStatus.RECEIVED,
];

const columns: Column[] = [
  { title: "Receipt No", value: "receipt_no", classname: "w-[16%]" },
  { title: "Date", value: "date", classname: "w-[12%]" },
  { title: "PO", value: "po", classname: "w-[14%]" },
...
export const GoodReceiptPage = ({ title, subtitle }: DataProps) => {
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<GoodReceiptApiDaum>>(
        "/good-receipt",
        ...
        <div className="flex justify-end mb-3">
          <div className="inline-flex flex-wrap items-center gap-1 rounded-xl border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/60 p-1">
            {[
              ...
              ...STATUS_RECEIPT.map((s) => ({
                value: s as string,
                label: GR_STATUS_LABEL[s],
