...
const STATUS_DELIVERY: DeliveryOrderStatus[] = [
  DeliveryOrderStatus.PENDING,
  DeliveryOrderStatus.SHIPPED,
];

const columns: Column[] = [
  { title: "Delivery No", value: "delivery_no", classname: "w-[16%]" },
  { title: "Date", value: "date", classname: "w-[12%]" },
  { title: "Warehouse", value: "warehouse", classname: "w-[18%]" },
  { title: "Recipient", value: "recipient", classname: "w-[16%]" },
...
export const DeliveryOrderPage = ({ title, subtitle }: DataProps) => {
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<DeliveryOrderApiDaum>>(
        "/delivery-order",
        ...
        <div className="flex justify-end mb-3">
          <div className="inline-flex flex-wrap items-center gap-1 rounded-xl border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/60 p-1">
            {[
              ...
              ...STATUS_DELIVERY.map((s) => ({
                value: s as string,
                label: DO_STATUS_LABEL[s],
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 pb-4 border-b border-gray-100 dark:border-zinc-700">
            <div className="flex flex-col space-y-1.5">
              ...
              <span className="text-xs text-gray-400 dark:text-zinc-500 select-none">
                ...
                in delivery no, recipient &amp; reference
              </span>
            </div>
          </div>
