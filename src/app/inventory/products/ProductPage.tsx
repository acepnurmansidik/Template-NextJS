...
import { ProductApiDaum } from "@/types/product";
import CreateProductModal from "@/components/atoms/modals/create/CreateProductModal";
import { TableProduct } from "@/components/atoms/table/tableProduct";
...
const columns: Column[] = [
  ...
  { title: "Category", value: "category", classname: "w-[16%]" },
  { title: "UOM", value: "uom", classname: "w-[14%]" },
  { title: "Sell Price", value: "selling_price", classname: "w-[14%]" },
  { title: "Status", value: "status", classname: "w-[8%]" },
  { title: "Action", value: "action", classname: "w-[8%]" },
];

export const ProductPage = ({ title, subtitle }: DataProps) => {
  ...
  const [data, setData] = useState<ProductApiDaum[]>([]);
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<ProductApiDaum>>(
        "/product",
        { page, limit, search },
        false,
      );
