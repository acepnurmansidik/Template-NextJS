...
import CreateUomModal from "@/components/atoms/modals/create/CreateUomModal";
import { TableUom } from "@/components/atoms/table/tableUom";
...
const columns: Column[] = [
  { title: "Code", value: "code", classname: "w-[15%]" },
  { title: "Name", value: "name", classname: "w-[25%]" },
...
export const UomPage = ({ title, subtitle }: DataProps) => {
  ...
  const [data, setData] = useState<UomApiDaum[]>([]);
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<UomApiDaum>>(
        "/uom",
        { page, limit, search },
        false,
      );
