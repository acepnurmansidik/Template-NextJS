...
import CreateWarehouseModal from "@/components/atoms/modals/create/CreateWarehouseModal";
import { TableWarehouse } from "@/components/atoms/table/tableWarehouse";
...
export const WarehousePage = ({ title, subtitle }: DataProps) => {
  ...
  const [data, setData] = useState<WarehouseApiDaum[]>([]);
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<WarehouseApiDaum>>(
        "/warehouse",
        { page, limit, search },
        false,
      );
