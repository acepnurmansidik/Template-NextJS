...
import { StockPositionApiDaum } from "@/types/stockPosition";
import CreateStockPositionModal from "@/components/atoms/modals/create/CreateStockPositionModal";
import { TableStockPosition } from "@/components/atoms/table/tableStockPosition";
...
const columns: Column[] = [
  { title: "Product", value: "product", classname: "w-[30%]" },
  { title: "Warehouse", value: "warehouse", classname: "w-[26%]" },
  { title: "Quantity", value: "quantity", classname: "w-[16%]" },
  { title: "Reserved", value: "reserved_quantity", classname: "w-[16%]" },
  { title: "Action", value: "action", classname: "w-[12%]" },
];

export const StockPositionPage = ({ title, subtitle }: DataProps) => {
  const currentUser = useAppSelector((state) => state.iam.data);
  const pathname = usePathname();
  const [hasAccess, setHasAccess] = useState<Record<string, boolean>>({});

  const [data, setData] = useState<StockPositionApiDaum[]>([]);
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<StockPositionApiDaum>>(
        "/stock-position",
        { page, limit, search },
        false,
      );
