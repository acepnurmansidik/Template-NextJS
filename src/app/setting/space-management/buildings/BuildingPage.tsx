...
import { BuildingApiDaum, ListResponse } from "@/types/facility";
import CreateBuildingModal from "@/components/atoms/modals/create/CreateBuildingModal";
import { TableBuilding } from "@/components/atoms/table/tableBuilding";
...
const columns = [
  ...
  { title: "Branch", value: "branch", classname: "w-[18%]" },
  { title: "Type", value: "building_type", classname: "w-[14%]" },
  { title: "Floors", value: "total_floors", classname: "w-[10%] text-center" },
  { title: "Status", value: "status", classname: "w-[10%]" },
  { title: "Action", value: "action", classname: "w-[14%]" },
];

export const BuildingPage = ({ title, subtitle }: DataProps) => {
  const currentUser = useAppSelector((state) => state.iam.data);
  const pathname = usePathname();
  const [hasAccess, setHasAccess] = useState<Record<string, boolean>>({});

  const [data, setData] = useState<BuildingApiDaum[]>([]);
