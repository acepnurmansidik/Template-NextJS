...
const columns = [
  ...
  { title: "Type", value: "type", classname: "w-[10%]" },
  { title: "Area", value: "floor_area_sqm", classname: "w-[8%] text-right" },
  { title: "Capacity", value: "max_capacity", classname: "w-[8%] text-right" },
  { title: "Plan", value: "plan", classname: "w-[6%] text-center" },
...
export const BuildingFloorPage = ({ title, subtitle }: DataProps) => {
  ...
  const fetchingData = useCallback(async () => {
    try {
      const result = await apiGet<ListResponse<BuildingFloorApiDaum>>(
        "/building-floor",
        { page, limit, search, building_id: buildingId },
        false,
      );
      setData(result.data ?? []);
  ...
  return (
    <CMSLayout>
      <div className="w-full px-6 transition-colors duration-300">
        ...
        <div className="px-6 py-4 rounded-lg shadow-xs bg-white dark:bg-zinc-800 transition-colors duration-300">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 pb-4 border-b border-gray-100 dark:border-zinc-700">
            ...
            {/* BUILDING FILTER */}
            <div className="w-64 self-start md:self-auto">
              <Select
                instanceId="floor-building-filter"
                classNamePrefix="rs"
                options={buildingOptions}
                value={
                  buildingOptions.find((o) => o.value === buildingId) ??
                  buildingOptions[0]
                }
                onChange={(opt) => {
                  setBuildingId(opt?.value ?? "");
                  setPage(1);
                }}
                menuPortalTarget={
      ...
      {isModalCreateOpen && (
        <CreateBuildingFloorModal
          isOpen={isModalCreateOpen}
          defaultBuildingId={buildingId || null}
