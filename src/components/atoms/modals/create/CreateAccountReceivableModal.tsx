...
export default function CreateAccountReceivableModal({
  isOpen,
  onClose,
  onSuccess,
}: DataProps) {
  ...
  const [formData, setFormData] = useState<FormDataAccountReceivableProps>(
    () => ({
      ...
      reference: "",
      description: "",
      status: AccountReceivableStatus.DRAFT,
      paid_amount: 0,
    }),
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiPost<SingleResponse<AccountReceivableApiDaum>>(
        "/account-receivable",
        payload,
        false,
        false,
                    ...
                    return (
                      ...
                      >
                        <td className="py-2 px-3 align-top">
                          <Select
                            instanceId={`ar-line-account-${index}`}
                            classNamePrefix="rs"
