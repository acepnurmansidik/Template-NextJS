...
import { AppConfigForm, UpdateType, AppConfigApiDaum } from "@/types/appConfig";
...
// react-select style agar konsisten dengan input lain (rounded-lg, border zinc,
// dark-mode) & menu tampil di atas segalanya.
const selectStyles = {
  menuPortal: (base: any) => ({ ...base, zIndex: 9999 }),
};

export default function CreateAppConfigModal({
  isOpen,
  onClose,
  onSuccess,
}: DataProps) {
  ...
  const handleSubmit = async () => {
    ...
    // --- Validasi format versi (semver: harus X.Y.Z) ---
    ...
    setIsLoading(true);
    try {
      // Platform dipaksa lower-case saat dikirim ke backend.
      ...
      const result = await apiPost<ListResponse<AppConfigApiDaum>>(
        "/app-configs",
        payload,
        false,
        false,
  ...
  // Platform: paksa lower-case saat mengetik.
  ...
  const handleVersionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Beri feedback validasi realtime, tapi jangan ganggu saat masih kosong.
        ...
        <div className="max-w-full px-5 mx-auto space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="group">
              ...
              <Select
                instanceId="appconfig-update-type"
                classNamePrefix="rs"
