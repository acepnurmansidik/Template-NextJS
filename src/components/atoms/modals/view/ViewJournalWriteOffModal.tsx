...
export default function ViewJournalWriteOffModal({
  isOpen,
  onClose,
  initialData,
}: DataProps) {
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center gap-3">
            ...
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border border-blue-300 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700">
              {WRITE_OFF_TYPE_LABEL[initialData.write_off_type ?? WriteOffType.OTHER]}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <Field
              label="Date"
              value={new Date(initialData.date).toLocaleDateString("id-ID", {
                day: "2-digit",
                month: "short",
                year: "numeric",
              })}
            ...
            <Field
              ...
              value={
                ...
                initialData.source_type === WriteOffSourceType.NONE
                  ? "—"
                  : `${
                      WRITE_OFF_SOURCE_LABEL[
                        initialData.source_type as WriteOffSourceType
                      ] ?? initialData.source_type
                    }: ${initialData.source_no || "—"}`
              }
            />
          </div>
              ...
              <tfoot>
                <tr className="border-t-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 font-bold text-sm">
                  ...
                  <td className="py-3 px-3 text-right font-mono text-zinc-800 dark:text-zinc-100">
                    {formatAmount(initialData.total_debit)}
                  </td>
                  <td className="py-3 px-3 text-right font-mono text-zinc-800 dark:text-zinc-100">
                    {formatAmount(initialData.total_credit)}
                  </td>
                </tr>
              </tfoot>
            </table>
