...
export default function ViewGoodReceiptModal({
  isOpen,
  onClose,
  initialData,
}: DataProps) {
      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center gap-3">
            ...
            >
              {GR_STATUS_LABEL[initialData.status] ?? initialData.status}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <Field
              label="Received Date"
              value={fmtDate(initialData.received_date)}
            />
            <Field
              label="Purchase Orders"
              value={poLabels.length > 0 ? poLabels.join(", ") : "—"}
            />
            <Field
            ...
            <Field
              label="Warehouse"
              value={isMulti ? "—" : refLabel(initialData.warehouse_id)}
            />
            <Field
              label="Reference"
            ...
            <table className="w-full text-left min-w-[760px]">
              <thead className="bg-zinc-50 dark:bg-zinc-800/60">
                <tr className="text-[11px] uppercase tracking-widest text-zinc-500">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Ordered</th>
                  ...
                  items.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-right font-mono text-zinc-800 dark:text-zinc-200">
                        {formatAmount(item.received_qty ?? 0)}
                      </td>
                    </tr>
                  ))
                )}
