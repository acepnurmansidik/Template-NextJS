...
import { ProductCategoryApiDaum, accountId } from "@/types/productCategory";

interface DataProps {
  isOpen: boolean;
  onClose: () => void;
  initialData: ProductCategoryApiDaum;
}
...
export default function ViewProductCategoryModal({
  isOpen,
  onClose,
  initialData,
}: DataProps) {
  ...
  const lineAccounts = initialData.line_accounts ?? [];
      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-zinc-500">{initialData.prefix}</span>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <Field label="Name" value={initialData.name} />
            <Field label="Prefix" value={initialData.prefix} />
            <Field
              label="Status"
              ...
              <table className="w-full text-left">
                <thead className="bg-zinc-50 dark:bg-zinc-900/50">
                  <tr className="text-xs text-zinc-500">
                    <th className="py-2 px-3 font-bold uppercase tracking-widest">
                      Title
                    </th>
                    <th className="py-2 px-3 font-bold uppercase tracking-widest">
                      Account
                ...
                <tbody>
                  {lineAccounts.length === 0 ? (
                    <tr>
                      <td
                        colSpan={2}
                        className="py-6 px-3 text-center text-sm text-zinc-400"
                      >
                        No line accounts.
                      </td>
                    </tr>
                  ) : (
                    lineAccounts.map((la, index) => {
                      const ref = la.account_id;
                      const account =
                        ref && typeof ref === "object"
                          ? [ref.code, ref.name].filter(Boolean).join(" — ") ||
                            "—"
                          : "—";
                      return (
                        <tr
                          key={la._id ?? accountId(ref) ?? index}
                          className="border-t border-zinc-100 dark:border-zinc-800/60 text-sm"
                        >
                          <td className="py-2 px-3 text-zinc-800 dark:text-zinc-200">
                            {la.title || "—"}
                          </td>
