"use client";

import { useEffect } from "react";
import { IoClose } from "react-icons/io5";
import {
  MovementType,
  MOVEMENT_TYPE_LABEL,
  refLabel,
  StockMovementApiDaum,
} from "@/types/stockMovement";
import { formatCurrencyPure } from "@/utils/formatter";

interface DataProps {
  isOpen: boolean;
  onClose: () => void;
  initialData: StockMovementApiDaum;
}

// Warna badge per tipe pergerakan stok (selaras dengan tabel).
const TYPE_BADGE: Record<MovementType, string> = {
  [MovementType.IN]:
    "border-emerald-300 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700",
  [MovementType.OUT]:
    "border-red-300 bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300 dark:border-red-700",
  [MovementType.ADJUSTMENT]:
    "border-amber-300 bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700",
  [MovementType.TRANSFER]:
    "border-sky-300 bg-sky-50 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-700",
};

const Field = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div>
    <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-1">
      {label}
    </p>
    <div className="text-sm text-zinc-800 dark:text-zinc-100">{value}</div>
  </div>
);

const fmtDate = (d?: string) =>
  d
    ? new Date(d).toLocaleDateString("id-ID", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      })
    : "—";

export default function ViewStockMovementModal({
  isOpen,
  onClose,
  initialData,
}: DataProps) {
  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === "Escape") onClose();
    };
    window.addEventListener("keydown", handleEsc);
    return () => window.removeEventListener("keydown", handleEsc);
  }, [onClose]);

  if (!isOpen) return null;

  const isTransfer = initialData.type === MovementType.TRANSFER;

  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-zinc-950">
      <div className="flex justify-between items-center px-8 py-6 border-b border-zinc-200 dark:border-zinc-800">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">
          Stock Movement Detail
        </h2>
        <button
          onClick={onClose}
          className="text-zinc-400 hover:text-zinc-900 flex items-center duration-300 justify-center dark:hover:text-zinc-100 text-sm font-medium hover:bg-zinc-300/20 rounded-md hover:cursor-pointer h-9 w-9"
        >
          <IoClose className="text-red-500 text-xl" />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center gap-3">
            <span className="font-medium text-base text-zinc-900 dark:text-zinc-100">
              {refLabel(initialData.product_id)}
            </span>
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                TYPE_BADGE[initialData.type] ?? ""
              }`}
            >
              {MOVEMENT_TYPE_LABEL[initialData.type] ?? initialData.type}
            </span>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <Field label="Date" value={fmtDate(initialData.date ?? initialData.created_at)} />
            <Field label="Product" value={refLabel(initialData.product_id)} />
            <Field label="UOM" value={refLabel(initialData.uom_id)} />
            <Field
              label={isTransfer ? "Source Warehouse" : "Warehouse"}
              value={refLabel(initialData.warehouse_id)}
            />
            {isTransfer && (
              <Field
                label="Destination Warehouse"
                value={refLabel(initialData.destination_warehouse_id)}
              />
            )}
            <Field
              label="Quantity"
              value={
                <span className="font-mono">
                  {formatCurrencyPure(initialData.quantity ?? 0)}
                </span>
              }
            />
            <Field label="Reference" value={initialData.reference?.trim() || "—"} />
            <Field label="Note" value={initialData.note?.trim() || "—"} />
          </div>
        </div>
      </div>

      <div className="px-8 py-6 border-t border-zinc-200 dark:border-zinc-800 flex justify-end gap-3 bg-zinc-50 dark:bg-zinc-900/50">
        <button
          onClick={onClose}
          className="px-6 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-200 rounded-lg"
        >
          Close
        </button>
      </div>
    </div>
  );
}
