...
export default function UpdateProductCategoryModal({
  ...
  const [lines, setLines] = useState<LineForm[]>(
    (initialData.line_accounts ?? []).map((l) => ({
      title: l.title ?? "",
      account_id: accountId(l.account_id),
    })),
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiPut<SingleResponse<ProductCategoryApiDaum>>(
        `/product-category/${initialData._id}`,
        payload,
        false,
        false,
      );
                      ...
                      return (
                        ...
                        >
                          <td className="py-2 px-3 align-top">
                            <Select
                              instanceId={`pc-coa-update-${index}`}
                              classNamePrefix="rs"
