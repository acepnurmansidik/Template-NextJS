...
export default function UpdateDeliveryOrderModal({
  isOpen,
  initialData,
  onClose,
  onSuccess,
  ...
  const [items, setItems] = useState<PurchaseItemForm[]>(() =>
    initialData.items?.length
      ? initialData.items.map((it) => apiItemToForm(it))
      : [{ ...emptyPurchaseItem }],
  );
  ...
  // Sisipkan warehouse awal bila belum ada di daftar (agar label tampil).
  const mergedWarehouseOptions = useMemo(() => {
    const initId = refId(initialData.warehouse_id);
    if (initId && !warehouseOptions.some((o) => o.value === initId)) {
      return [
        { value: initId, label: refCodeName(initialData.warehouse_id) || initId },
        ...warehouseOptions,
      ];
    }
    return warehouseOptions;
  }, [warehouseOptions, initialData.warehouse_id]);
  const selectedWarehouse =
    mergedWarehouseOptions.find((o) => o.value === warehouseId) ?? null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiPut<SingleResponse<DeliveryOrderApiDaum>>(
        `/delivery-order/${initialData._id}`,
        payload,
        false,
        false,
      );
        ...
        <div className="max-w-full px-5 mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="group">
              ...
              <Select
                instanceId="do-warehouse-update"
                classNamePrefix="rs"
                  ...
                  {items.map((item, index) => (
                    ...
                    >
                      <td className="py-2 px-3 align-top">
                        <ProductAsyncSelect
                          instanceId={`do-item-product-upd-${index}`}
