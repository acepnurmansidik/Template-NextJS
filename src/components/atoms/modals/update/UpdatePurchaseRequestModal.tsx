...
interface DataProps {
  isOpen: boolean;
  initialData: PurchaseRequestApiDaum;
  onClose: () => void;
  onSuccess?: () => void;
}

export default function UpdatePurchaseRequestModal({
  isOpen,
  initialData,
  onClose,
  onSuccess,
  ...
  const [formData, setFormData] = useState<FormDataPurchaseRequestProps>(() => ({
    ...
    needed_date: initialData.needed_date?.slice(0, 10) ?? "",
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const { date, needed_date, requested_by, reference, description, status } =
        formData;
      ...
      const result = await apiPut<SingleResponse<PurchaseRequestApiDaum>>(
        `/purchase-request/${initialData._id}`,
        payload,
        false,
        false,
      );
                    ...
                    return (
                      ...
                      >
                        <td className="py-2 px-3 align-top">
                          <ProductAsyncSelect
                            instanceId={`pr-item-product-upd-${index}`}
