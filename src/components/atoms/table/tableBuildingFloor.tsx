...
export const TableBuildingFloor = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const handleDelete = async (row: BuildingFloorApiDaum) => {
    try {
      const confirmation = await Swal.fire({
        title: "Are you sure?",
        text: "Lantai beserta ruangan di dalamnya akan ikut dihapus.",
        icon: "warning",
      ...
      const result = await apiDelete<SingleResponse<BuildingFloorApiDaum>>(
        `/building-floor/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const renderCell = (row: BuildingFloorApiDaum, value: string) => {
    switch (value) {
      case "code":
        return (
      ...
      case "building":
        return refName(row.building_id);
      case "type":
        return (
          <span className="text-xs text-gray-600 dark:text-zinc-400 capitalize">
            {row.type}
          </span>
        );
      case "floor_level":
      ...
      case "plan":
        return refImagePath(row.floor_plan_url_id) ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no floors found. Try creating a new entry or
