...
export const TableProductCategory = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const handleDelete = async (row: ProductCategoryApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<ProductCategoryApiDaum>>(
        `/product-category/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const renderCell = (row: ProductCategoryApiDaum, value: string) => {
    switch (value) {
      case "prefix":
        return (
      ...
      case "accounts":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border border-sky-300 bg-sky-50 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-700">
            {row.line_accounts?.length ?? 0} account
            {(row.line_accounts?.length ?? 0) === 1 ? "" : "s"}
          </span>
        );
      case "status":
        return (
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no product categories found. Try creating a
