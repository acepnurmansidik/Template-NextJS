...
// Warna badge per tipe pergerakan stok.
...
export const TableStockMovement = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const handleDelete = async (row: StockMovementApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<StockMovementApiDaum>>(
  ...
  const renderCell = (row: StockMovementApiDaum, value: string) => {
    switch (value) {
      case "date":
        return (
      ...
      case "type":
        return (
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${TYPE_BADGE[row.type]}`}
          >
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no stock movements found. Try creating a new
