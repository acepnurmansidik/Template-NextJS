...
import UpdateSupplierPricingModal from "../modals/update/UpdateSupplierPricingModal";
import ViewSupplierPricingModal from "../modals/view/ViewSupplierPricingModal";

interface DataProps {
  columns: Column[];
  hasAccess: Record<string, boolean>;
  data: SupplierPricingApiDaum[];
  page: number;
  limit: number;
  totalData: number;
  totalPage: number;
...
export const TableSupplierPricing = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const [selectedData, setSelectedData] =
    useState<SupplierPricingApiDaum | null>(null);

  const handleDelete = async (row: SupplierPricingApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<SupplierPricingApiDaum>>(
        `/supplier-pricing/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const renderCell = (row: SupplierPricingApiDaum, value: string) => {
    switch (value) {
      case "name":
        return (
      ...
      case "status":
        return row.is_active ? (
          ...
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border border-zinc-300 bg-zinc-50 text-zinc-600 dark:bg-zinc-700/40 dark:text-zinc-400 dark:border-zinc-600">
            Inactive
          </span>
        );
      default:
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There is no supplier pricing yet. Try creating a new
