...
export const TableBranch = ({
  ...
  const [selectedData, setSelectedData] = useState<BranchApiDaum | null>(null);
  const [showModalUpdate, setShowModalUpdate] = useState(false);
  const [showModalView, setShowModalView] = useState(false);

  const handleDelete = async (row: BranchApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<BranchApiDaum>>(
        `/branch/${row._id}`,
        {},
        false,
      );
  ...
  const renderCell = (row: BranchApiDaum, value: string) => {
    switch (value) {
      case "code":
        return (
      ...
      case "manager":
        return row.contact_info?.manager_name || "—";
      case "phone":
        return row.contact_info?.phone?.[0] || "—";
      case "status":
        return (
          <span
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no branches found. Try creating a new entry
