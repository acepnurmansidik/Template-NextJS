...
import { GoodReceiptApiDaum, GR_STATUS_LABEL } from "@/types/goodReceipt";
import { GoodReceiptStatus, refLabel } from "@/types/purchaseItem";
import { STATUS_BADGE } from "@/utils/utils";
import { formatCurrencyPure } from "@/utils/formatter";
import UpdateGoodReceiptModal from "../modals/update/UpdateGoodReceiptModal";
import ViewGoodReceiptModal from "../modals/view/ViewGoodReceiptModal";

interface DataProps {
  columns: Column[];
  hasAccess: Record<string, boolean>;
  data: GoodReceiptApiDaum[];
  page: number;
  limit: number;
  totalData: number;
  totalPage: number;
...
export const TableGoodReceipt = ({
  ...
  const [selectedData, setSelectedData] = useState<GoodReceiptApiDaum | null>(
    null,
  );

  const handleDelete = async (row: GoodReceiptApiDaum) => {
    try {
      const confirmation = await Swal.fire({
        title: "Are you sure?",
        text: "Stok yang sudah masuk dari GR ini akan dikembalikan.",
        icon: "warning",
      ...
      const result = await apiDelete<SingleResponse<GoodReceiptApiDaum>>(
        `/good-receipt/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const renderCell = (row: GoodReceiptApiDaum, value: string) => {
    switch (value) {
      case "receipt_no":
        return (
      ...
      case "status":
        return (
          ...
          >
            {GR_STATUS_LABEL[row.status] ?? row.status}
          </span>
        );
      default:
        return "—";
            ...
            <tbody>
              {data.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no good receipts found. Try creating a new
                ...
                data.map((row, rowIndex) => {
                  const locked = row.status === GoodReceiptStatus.RECEIVED;
                  return (
                    <tr
                      key={row._id}
