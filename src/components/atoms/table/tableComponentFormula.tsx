...
export const TableComponentFormula = ({
  ...
  const handleModalView = (newData: ComponentFormulaApiDaum) => {
    setShowModalView(true);
    setSelectedData(newData);
  };
  const handleModalUpdate = (newData: ComponentFormulaApiDaum) => {
                      ...
                      .map((col, indexCol) => (
                        ...
                        >
                          {col.value === "*" ? (
                            ...
                            <span
                              className={`inline-flex text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${rateTypeBadge(
                                row.rate_type,
                              )}`}
                            >
                              {row.rate_type}
                            ...
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border border-zinc-300 bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-600">
                              {row.component_id?.length ?? 0} formula
                            </span>
                          ) : col.value === "name" ? (
