...
import UpdateSupplierModal from "../modals/update/UpdateSupplierModal";
import ViewSupplierModal from "../modals/view/ViewSupplierModal";

interface DataProps {
  columns: Column[];
  hasAccess: Record<string, boolean>;
  data: SupplierApiDaum[];
  page: number;
  limit: number;
  totalData: number;
  totalPage: number;
...
export const TableSupplier = ({
  ...
  const [selectedData, setSelectedData] = useState<SupplierApiDaum | null>(
    null,
  );

  const handleDelete = async (row: SupplierApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<SupplierApiDaum>>(
        `/supplier/${row._id}`,
        {},
        false,
      );
  ...
  const renderCell = (row: SupplierApiDaum, value: string) => {
    switch (value) {
      case "code":
        return (
      ...
      case "contact_person":
        return (
          <span className="text-gray-600 dark:text-zinc-400">
            {row.contact_info?.contact_person || "—"}
          </span>
        );
      case "phone":
        return (
          <span className="text-gray-600 dark:text-zinc-400">
            {row.contact_info?.phone?.[0] || "—"}
          </span>
        );
      case "status":
        return row.is_active ? (
          ...
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border border-rose-300 bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300 dark:border-rose-700">
            Inactive
          </span>
        );
      default:
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no suppliers found. Try creating a new entry
