...
export const TableStockPosition = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const handleDelete = async (row: StockPositionApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<StockPositionApiDaum>>(
        `/stock-position/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const renderCell = (row: StockPositionApiDaum, value: string) => {
    switch (value) {
      case "product":
        return (
      ...
      case "reserved_quantity":
        return (
          <span className="font-mono text-gray-800 dark:text-zinc-200">
            {formatCurrencyPure(row.reserved_quantity ?? 0)}
          </span>
        );
      default:
        return "—";
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no stock positions found. Try creating a new
