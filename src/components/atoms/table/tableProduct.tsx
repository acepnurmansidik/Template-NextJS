...
export const TableProduct = ({
  ...
  const handleDelete = async (row: ProductApiDaum) => {
    try {
      ...
      const result = await apiDelete<SingleResponse<ProductApiDaum>>(
        `/product/${row._id}`,
        {},
        false,
      );
  ...
  const renderCell = (row: ProductApiDaum, value: string) => {
    switch (value) {
      case "code":
        return (
          <span className="font-mono font-bold text-gray-800 dark:text-zinc-100">
            {row.code}
          </span>
        );
      case "name":
      ...
      case "category":
        return (
          <span className="text-gray-600 dark:text-zinc-400">
            {refLabel(row.product_category_id)}
          </span>
        );
      case "uom":
        return (
      ...
      case "selling_price":
        return (
          <span className="tabular-nums text-gray-700 dark:text-zinc-300">
            {priceFmt.format(row.selling_price ?? 0)}
          </span>
        );
      case "status":
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no products found. Try creating a new entry
