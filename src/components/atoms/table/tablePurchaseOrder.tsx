...
import UpdatePurchaseOrderModal from "../modals/update/UpdatePurchaseOrderModal";
import ViewPurchaseOrderModal from "../modals/view/ViewPurchaseOrderModal";

interface DataProps {
  columns: Column[];
  hasAccess: Record<string, boolean>;
  data: PurchaseOrderApiDaum[];
  page: number;
  limit: number;
  totalData: number;
  totalPage: number;
...
export const TablePurchaseOrder = ({
  hasAccess,
  columns,
  data,
  page,
  ...
  const [selectedData, setSelectedData] = useState<PurchaseOrderApiDaum | null>(
    null,
  );

  const handleDelete = async (row: PurchaseOrderApiDaum) => {
    try {
      if (row.status === ProcurementStatus.SUBMITTED) {
        Swal.fire({
          icon: "info",
          title: "Submitted order",
          text: "Submitted purchase orders cannot be deleted.",
      ...
      const result = await apiDelete<SingleResponse<PurchaseOrderApiDaum>>(
        `/purchase-order/${row._id}`,
        {},
        false,
      );
      if (result.success) {
  ...
  const handleSubmit = async (row: PurchaseOrderApiDaum) => {
    try {
      const confirmation = await Swal.fire({
      ...
      if (result.success) {
        await Swal.fire({
          ...
          text: result.message || "Purchase order has been submitted.",
          confirmButtonText: "OK",
          confirmButtonColor: "#2563eb",
          timer: 2500,
          timerProgressBar: true,
  ...
  const renderCell = (row: PurchaseOrderApiDaum, value: string) => {
    switch (value) {
      case "order_no":
        return (
                    ...
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 max-w-xs mx-auto">
                          There are no purchase orders found. Try creating a new
